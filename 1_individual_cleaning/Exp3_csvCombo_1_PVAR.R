#------------------#
  #title: "16x16_CSV_Cleaner_Script_Exp3_Time"
  #author: "A Sampson"
...
##----- Set participant and Version -----
  ## input subject number_date (date in DDMMYY format) and version number  
  ## these will populate file names and graphic titles  
  
Subject <- "P16317_291124"
Version <- "1"
...
#  add_column(Rep = 0, .before = 'Trial') %>% ADD this after 
  ...
  add_column(Sbj = (Subject), .before = 'Block')
...
write_csv(dupes_all, (paste(Subject, Version, 'all_dupes.csv', sep ='_')))

#remove rows once decided which based on duplicates - remove hash tag, fill in row numbers and continue

#detect_1 <- detect_1[-c(81),] #these are the row numbers - good
#detect_2 <- detect_2[-c(221),] #these are the row numbers 
detect_3 <- detect_3[-c(291),] #these are the row numbers
#detect_4 <- detect_4[-c(221),] #these are the row numbers
#detect_5 <- detect_5[-c(60, 291),] #these are the row numbers
detect_6 <- detect_6[-c(76),] #these are the row numbers
#detect_T <- detect_T[-c(4,5,1),] #these are the row numbers


#16317 has dupes in Blocks 3 and 6 and don't forget to save
...
detect_T <- detect_T %>% 
  select(Sbj,Block, TrialNum, FileName, Cond, Location, Onset, Condition, Onset_T, Side, Code_Response, Code_old_new_response, Code_time_response, Time_Trial_Cue, Time_Trial_Target, Time_Response, Time_Trial_Q_ON,Time_old_new_response, Time_Trial_Q_Time, Time_time_response, Time_Trial_End, TTime_Trial_Cue, TTime_Trial_Target, TTime_Response, TTime_old_new_response, TTime_time_response)
...
detect_1$Onset_T [ detect_1$Onset_T == "L"] <- "2000"
detect_1$Onset_T [ detect_1$Onset_T == "R"] <- "2000"
...
detect_2$Onset_T [ detect_2$Onset_T == "L"] <- "2000"
detect_2$Onset_T [ detect_2$Onset_T == "R"] <- "2000"
...
detect_3$Onset_T [ detect_3$Onset_T == "L"] <- "2000"
detect_3$Onset_T [ detect_3$Onset_T == "R"] <- "2000"
#add the exposure number
...
detect_4$Onset_T [ detect_4$Onset_T == "L"] <- "2000"
detect_4$Onset_T [ detect_4$Onset_T == "R"] <- "2000"
...
detect_5$Onset_T [ detect_5$Onset_T == "L"] <- "2000"
detect_5$Onset_T [ detect_5$Onset_T == "R"] <- "2000"
...
detect_6$Onset_6 [ detect_6$Onset_T == "L"] <- "2000"
detect_6$Onset_6 [ detect_6$Onset_T == "R"] <- "2000"
...
#Location accuracy
detect_exp <- detect_exp %>% 
  mutate(ACC_Time = case_when(
    Resp_Time == 3 & StimCon == 1 ~ 1,
    Resp_Time == 3 & StimCon == 2 ~ 1,
    Resp_Time == 3 & StimCon >= 3 ~ 0,
    Resp_Time == 4 & StimCon == 3 ~ 1,
    Resp_Time == 4 & StimCon == 4 ~ 1,
    Resp_Time == 4 & StimCon <= 2 ~ 0,
    Resp_Time == 4 & StimCon >= 5 ~ 0,
    Resp_Time == 6 & StimCon >= 5 ~ 1,
    Resp_Time == 6 & StimCon <= 4 ~ 0,
  ))
...
detect_exp$Resp_Time <- recode_factor(
  detect_exp$Resp_Time,
  `3` = "same",
  `4` = "diff",
  `6` = "new",
  .default = "NA"
...
#arrange columns
detect_exp <- detect_exp %>% 
  select(Sbj, Block, TrialNum, Wav, Cond, Side,Location, Onset, Resp, rt, ACC, Resp_Old, rt_Old, ACC_ON, Resp_Time, rt_Time, ACC_Time, TTime, KTime, Time_Trial_Target, Time_Response, StimCon, FileName,  RT_Chk, Time_Trial_Cue, Time_Trial_Target, Time_Response,  Time_Trial_Q_ON, TTime_Trial_Cue, TTime_time_response, Time_Trial_Scene, TTime_Trial_Scene, Ver, Rep, RmSq)  
...
detect_exp <- detect_exp %>% 
  select(Sbj, Block, TrialNum, Wav, Cond, Side,Location, Onset, Resp, rt, ACC, Resp_Old, rt_Old, ACC_ON, Resp_Time, rt_Time, ACC_Time, TTime, KTime, ex_rt_median, ex_rt_stdev, ex_rt_Outlier, Time_Trial_Target, Time_Response, StimCon, FileName,  RT_Chk, Time_Trial_Cue, Time_Trial_Q_ON, TTime_Trial_Cue, TTime_time_response, Ver, Rep, RmSq)  

arrange(detect_exp, TrialNum)

#export Comb csv for visualizing and analysis
write_csv(detect_exp, (paste(Subject, Version, 'COMBO_Cleaned.csv', sep = '_'))) ## ALL good
save(detect_exp, file= (paste(Subject, Version, '.RData', sep = '_')))
...
FLT_datp_Block_scat <- ggplot(detect_exp, aes(x = Block, y = rt, fill = Cond, color=Cond)) + geom_point() + labs(title = (paste("Clean rt", Subject, Version, sep = "_", x = "Block", y = "rt in milliseconds"))) + theme_grey() + geom_jitter() + theme(
...
FLT_cond_block <- ggplot(detect_exp_plt, aes(x = Block, y = rt, fill = Cond)) +  geom_boxplot()  + labs(title = (paste("Condition by Block", Subject, Version, sep = ' ')), x = "Block", y = "rt in milliseconds")  + theme(
  axis.text.x = element_text( color ="black", size = 9, angle= 90),
  axis.text.y = element_text( color="black", size=8, angle = 0))
FLT_cond_block
...
rt_summary <- 
  rt_summary |>
  tab_header(
    title = md(paste(Subject, "**Mean, Median & SD AVG rt by Condition**", sep ="")),
...
FLT_datp_Block_scat <- ggplot(detect_exp_plt, aes(x = Block, y = rt, fill = Cond, color=Cond)) + geom_point() + labs(title = (paste("Clean rt", Subject, Version, sep = "")), x = "Block", y = "rt in milliseconds") + theme_grey() + geom_jitter() + theme(
 axis.text.x = element_text( color ="black", size = 9, angle= 90),
  axis.text.y = element_text( color="black", size=8, angle = 0))
FLT_datp_Block_scat
...
FLT_cond <- ggplot(detect_exp_con, aes(x = Cond, y = rt, fill = Cond)) +  geom_boxplot()  + labs(title = (paste("Condition Only by Block", Subject, Version, sep = '')), x = "Block", y = "rt in milliseconds")  + theme(
  axis.text.x = element_text( color ="black", size = 9, angle= 90),
  axis.text.y = element_text( color="black", size=8, angle = 0))
FLT_cond
...
detect_exp_plt_sum <- 
  detect_exp_plt_sum |>
  tab_header(
    title = md(paste(Subject, Version, "**Mean, Median & SD AVG rt by Block & Condition**", sep = '')),
    subtitle =md("All Correct Exposure Trials")
  )
...
rt_summary <- 
  rt_summary |>
  tab_header(
    title = md(paste(Subject, Version, "** Mean, Median & SD AVG rt by Condition**", sep ='')),
    subtitle =md("All Correct Exposure Trials")
  )
...
Test_rt_ACC <- 
  ...
  tab_header(
    ...
    subtitle =md(paste("Participant", Subject, Version, sep = ' '))
  )
Test_rt_ACC 
...
Loc_tbl <- proportions(table(detect_exp$ACC_Time))
...
Loc_tbl <- 
  Loc_tbl |>
  tab_header(
    title = md("**Onset Test Accuracy Proportions ALL Condiiton**"),
    subtitle =md(paste("Participant", Subject, Version, sep =' '))
  )
Loc_tbl
Loc_tbl |> gtsave('ALL_Time_Prop.png')

test_tbl <-proportions(table(detect_exp$ACC, detect_exp$ACC_ON, detect_exp$ACC_Time))
test_tbl
...
(plot_3 <- ggplot(data = detect_exp_plt, aes(x = Rep, y = rt, fill = Cond, color = Cond)) +
    geom_violin(alpha = .5) +
...
ggsave((paste(Subject, Version, 'Violin_Exp_Block_rt.jpeg', sep = '_')), units = 'in', width = 5.5, height = 6)
